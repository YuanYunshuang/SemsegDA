...
class Unet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.hooks = [Hook(layer) for layer in list(self.modules()) if isinstance(layer, nn.ReLU)][:7]
        #self.out = None
    ...
    def set_input(self, image, label):
        self.A = image.to(self.device)
        self.B = label.to(self.device)

    def forward(self):
        """Standard forward"""
        self.out = self.model(self.A)

    def backward(self):
        self.loss = self.criterion(self.out, self.B)
        self.loss.backward()
    ...
    def inference(self, out):
        self.forward()
        self.loss = self.criterion(out, self.B)
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            ...
            if use_dropout:
                down = [downconv, nn.Dropout2d(0.1), downrelu]
                up = [upconv, nn.Dropout2d(0.1), uprelu]
            else:
                down = [downconv, downrelu]
                up = [upconv, uprelu]
            model = down + up
